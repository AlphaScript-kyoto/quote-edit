...
if not exist "%DATA_SRC%\company.json" (
  echo.
  echo ERROR: system\data\company.json is missing.
  echo        Copy company.example.json to company.json and fill contacts
  echo        before building. Do not commit company.json to Git.
  echo.
  goto :error
)
